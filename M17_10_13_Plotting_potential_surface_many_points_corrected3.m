...
for t=0:pi/4:2*pi  
   ...
   scatter3(position(:,1),position(:,2),position(:,3));
        sf=fit([position(:,1), position(:,2)], position(:,3), 'poly44');    %fits surface to potential
        plot(sf,[position(:,1), position(:,2)], position(:,3));
        figure
        [fj, fk] = differentiate(sf, [j, k])                              %Electric field calculated as derivative of potential
end
