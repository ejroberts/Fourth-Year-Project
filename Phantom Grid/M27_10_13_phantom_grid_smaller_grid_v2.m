...
for t=0:pi/4:2*pi  
   ...
   for j=1:1:n
       for k=1:1:n
         ...
         fprintf('%f',t);
         potentialt=(0.9)*A(j,k)+0.0001*(rand-0.5);         %Random walk based on potential at previous time
         A(j,k)=potentialt                                  %potential at each grid point
         position(l,3)=potentialt                           %Stores potential in an array related to a grid point
